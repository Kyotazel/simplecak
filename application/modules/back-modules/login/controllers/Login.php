...
class Login extends BackendController
{
    ...
    public function do_login()
    {
        ...
        if (!empty($get_data_user)) {
            ...
            if ($get_data_user->password == $hash_password) {
                ...
                // create token
                $token_id = time();
                Modules::run('database/update', 'st_user', ['id' => $get_data_user->id], ['token' => $token_id, 'forgot_password' => '']);
    ...
    public function update_image()
    {
        ...
        if (isset($_FILES['upload_background']) && !empty($_FILES['upload_background']['name'])) {
            ...
            Modules::run('database/update', 'app_setting', ['field' => 'admin_login_background'], $array_update);
        }

        if (isset($_FILES['upload_image']) && !empty($_FILES['upload_image']['name'])) {
            $image_name = $this->upload_image('upload_image');
            $array_update = ['value' => $image_name];
            Modules::run('database/update', 'app_setting', ['field' => 'admin_login_image'], $array_update);
        }
    ...
    public function delete_image()
    {
        ...
        if ($type == 'background') {
            $get_image = Modules::run('database/find', 'app_setting', ['field' => 'admin_login_background'])->row()->value;
            $dir = realpath(APPPATH . '../upload/banner');
            if (file_exists($dir . '/' . $get_image)) {
                unlink($dir . '/' . $get_image);
            }

            Modules::run('database/update', 'app_setting', ['field' => 'admin_login_background'], ['value' => '']);
        }
        if ($type == 'image') {
            $get_image = Modules::run('database/find', 'app_setting', ['field' => 'admin_login_image'])->row()->value;
            $dir = realpath(APPPATH . '../upload/banner');
            if (file_exists($dir . '/' . $get_image)) {
                unlink($dir . '/' . $get_image);
            }
            Modules::run('database/update', 'app_setting', ['field' => 'admin_login_image'], ['value' => '']);
        }
        echo json_encode(['status' => true]);
    }
}
