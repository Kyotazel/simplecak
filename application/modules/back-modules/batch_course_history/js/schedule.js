...
$(document).ready(function () {
    ...
    table = $('#table_data').DataTable({
        "ajax": {
            "url": url_controller + "batch_course_history/schedule/list_data/" + id_batch + "?token=" + _token_user,
            "type": "POST",
        },
        "columnDefs": [
            {
...
$('.btn_save').click(function (e) {
    ...
    $.ajax({
        url: url_controller + "batch_course_history/schedule/" + url + '?token' + _token_user,
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
$(document).on('click', '.btn_delete', function () {
    ...
    swal({
        ...
        function (isConfirm) {
            if (isConfirm) {
                $.ajax({
                    url: url_controller + "batch_course_history/schedule/"  + 'delete_data' + '?token=' + _token_user,
                    type: "POST",
                    dataType: "JSON",
                    data: { 'id': id },
                    success: function (data) {
