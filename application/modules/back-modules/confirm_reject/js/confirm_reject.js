...
$(document).on('click', '.btn_update_peserta', function () {
    id = $(this).data("id");
    status_peserta = $("#status_" + id).val();
    $.ajax({
        url: url_controller + "update_status_peserta" + "?token=" + _token_user,
        type: "POST",
        data: {id: id, status: status_peserta},
        dataType: "JSON",
        success: function(data) {
            notif({
                msg: `<b>Sukses : </b> Status Peserta berhasil diubah`,
                type: "success"
            })
            table_peserta.ajax.reload(null, false);
            table.ajax.reload(null, false);
