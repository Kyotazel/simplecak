...
class Confirm_reject extends BackendController
{
    var $module_name = 'confirm_reject';
    var $module_directory = 'confirm_reject';
    var $module_js = ['confirm_reject'];
    var $app_data = [];
    ...
    {
        $this->app_data['status'] = Modules::run('database/find', 'app_module_setting', ['params' => 'account_batch_status'])->result();
        $this->app_data['page_title']   = 'Peserta Yang ditolak';
    ...
    public function list_data()
    {
        ...
        foreach ($get_all as $data_table) {
            ...
            $array_peserta = [
                ...
                "where" => "id_batch_course = $data_table->id AND status = 10"
            ];
    ...
    {
        Modules::run("security/is_ajax");
        $status = Modules::run('database/find', 'app_module_setting', ['params' => 'account_batch_status'])->result();
        $id_batch_course = $this->input->post("id_batch_course");

        $get_all = Modules::run("database/find", "tb_batch_course_has_account", ["id_batch_course" => $id_batch_course, 'status' => 10])->result();
        ...
        foreach ($get_all as $data_table) {
            ...
            $data_status = "<select name='status_$data_table->id' id='status_$data_table->id' class='form-control'>";

            foreach($status as $value) {
                if ($value->value == $data_table->status) {
                    $data_status .= "<option value='$value->value' selected>$value->label</option>";
                } else {
                    $data_status .= "<option value='$value->value'>$value->label</option>";
                }
            }
            ...
            $row[] = $data_status;
            $row[] = "<button class='btn btn-success btn-sm btn_update_peserta' data-id=$data_table->id>Update</button>";
            $data[] = $row;
        }
