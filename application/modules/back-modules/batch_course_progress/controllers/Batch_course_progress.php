...
class Batch_course_progress extends BackendController
{
    var $module_name = 'batch_course_progress';
    var $module_directory = 'batch_course_progress';
    var $module_js = ['batch_course_progress'];
    var $app_data = [];
    ...
    {
        $this->app_data['page_title'] = 'Pelatihan sedang berlangsung';
    ...
    public function list_data() {
        ...
        $array_get = [
            ...
            "where" => "starting_date <= '" . date('Y-m-d') . "' AND ending_date >= '" . date('Y-m-d') . "'"
        ];
        ...
        foreach($get_all as $data_table) {
            ...
            $btn_edit       = Modules::run('security/edit_access', ' <a href="' . Modules::run('helper/create_url', 'batch_course_progress/edit?data=' . urlencode($this->encrypt->encode($data_table->id))) . '" data-id="' . $id_encrypt . '" class="btn btn-sm btn-success"><i class="fas fa-edit"></i> </a>');
            ...
            $btn_detail     = ' <a href="' . Modules::run('helper/create_url', 'batch_course_progress/detail?data=' . urlencode($this->encrypt->encode($data_table->id))) . '" class="btn btn-sm btn-info"><i class="fa fa-tv"></i></a> ';
            $btn_schedule   = " <a href='" . Modules::run('helper/create_url', 'batch_course_progress/schedule/index/'. $data_table->id) . "' class='btn btn-sm btn-outline-info text-info'><i class='fa fa-info-circle'></i> Detail</a>";
    ...
    public function save() 
    {
        ...
        $redirect = Modules::run('helper/create_url', 'batch_course_progress');
