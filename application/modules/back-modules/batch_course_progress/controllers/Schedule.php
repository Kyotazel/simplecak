...
class Schedule extends BackendController
{
    ...
    public function list_data($id)
    {
        ...
        foreach ($get_all as $data_table) {
            ...
            $btn_edit       = Modules::run('security/edit_access', ' <a href="' . Modules::run('helper/create_url', "batch_course_progress/schedule/edit/$id?data=" . urlencode($this->encrypt->encode($data_table->id))) . '" data-id="' . $id_encrypt . '" class="btn btn-sm btn-success"><i class="fas fa-edit"></i> </a>');
            ...
            $btn_detail     = ' <a href="' . Modules::run('helper/create_url', 'batch_course_progress/schedule/detail?data=' . urlencode($this->encrypt->encode($data_table->id))) . '" class="btn btn-sm btn-info"><i class="fa fa-tv"></i></a> ';
    ...
    public function list_peserta()
    {
        ...
        $get_batch_course = Modules::run("database/find", "tb_batch_course_has_account", ["id_batch_course" => $get_all->id_batch_course, "status" => 5])->result();
    ...
    public function validate_save()
    {
        ...
        if ($this->input->post('id_batch_course') == '') {
            ...
            $data['inputerror'][] = 'id_batch_course';
            $data['status'] = FALSE;
        }
    ...
    public function save()
    {
        ...
        $redirect = Modules::run('helper/create_url', "batch_course_progress/schedule/index/$id_batch_course");
