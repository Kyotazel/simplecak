...
class Security extends CommonController
{
    ...
    public function module_access_validation($controller)
    {
        ...
        $array_module_access = $this->session->userdata('us_credetial_module');
        $common_module = $this->config->item('common_module');
        $admin_module = $this->config->item('app_admin_module');
        if (!in_array($controller, $array_module_access) && !in_array($controller, $common_module) && !isset($admin_module[$controller]) && !$this->session->userdata('us_credential_admin')) {
    ...
    {
        $admin_module = $this->config->item('app_admin_module');
        $admin_access = $this->session->userdata('us_credential_admin');
    ...
    public function is_logged()
    {
        ...
        $id_user    = $this->session->userdata('us_id');
        $get_data   = Modules::run('database/find', 'st_user', ['id' => $id_user])->row();
        if (!empty($get_data) && !empty($token) && $token == $get_data->token) {
            //create url
    ...
    {
        $credential_access = $this->session->userdata('us_credential_access');
