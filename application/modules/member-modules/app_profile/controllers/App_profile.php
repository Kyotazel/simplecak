...
class App_profile extends BackendController
{
    ...
    public function index()
    {
        ...
        $get_data_user = Modules::run('database/find', 'mst_customer', ['id' => $id])->row();
    ...
    private function validate_update_profile()
    {
        ...
        if ($this->input->post('email') == '') {
            ...
            $get_data = Modules::run('database/find', 'mst_customer', ['email' => $email])->row();
        ...
        if ($this->input->post('password') == '') {
            ...
            if ($get_data_user->password != $hash_password) {
                $data['error_string'][] = 'password tidak valid';
                $data['inputerror'][] = 'password';
                $data['status'] = FALSE;
            }
        }
    ...
    private function validate_update_login()
    {
        ...
        if ($this->input->post('username') != '') {
            ...
            $get_data = Modules::run('database/find', 'mst_customer', ['username' => $username])->row();
            if (!empty($get_data) && $get_data->id != $id) {
        ...
        if ($this->input->post('password_current') == '') {
            $data['error_string'][] = 'Harus Diisi';
            $data['inputerror'][] = 'password_current';
            $data['status'] = FALSE;
        } else {
            $id = $this->input->post('id');
            $get_data_user = Modules::run('database/find', 'mst_customer', ['id' => $id])->row();
