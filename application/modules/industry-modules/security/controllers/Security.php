...
class Security extends CommonController
{
    ...
    {
        $token      = $this->session->userdata('industry_token_login');
        $id_user    = $this->session->userdata('industry_id');
        $get_data   = Modules::run('database/find', 'tb_industry', ['id' => $id_user])->row();
    ...
    public function module_access_validation($controller)
    {
        ...
        $array_module_access = $this->session->userdata('industry_credetial_module');
        $common_module = $this->config->item('common_module');
        $admin_module = $this->config->item('app_admin_module');
        if (!in_array($controller, $array_module_access) && !in_array($controller, $common_module) && !isset($admin_module[$controller]) && !$this->session->userdata('industry_credential_admin')) {
    ...
    {
        $admin_module = $this->config->item('app_admin_module');
        $admin_access = $this->session->userdata('industry_credential_admin');
