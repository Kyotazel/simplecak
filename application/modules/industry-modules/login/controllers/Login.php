    ...
    public function do_login()
    {
        ...
        if (!empty($get_data_user)) {
            ...
            if ($get_data_user->password == $hash_password) {
                ...
                // create token
                $token_id = time();
                Modules::run('database/update', 'tb_industry', ['id' => $get_data_user->id], ['token' => $token_id]);

                //create session
                $get_group_menu = Modules::run('database/find', 'app_menu', ['is_industry_menu' => 1])->row();
                $session_data = [
                    'industry_data' => $get_data_user,
                    'industry_credential_menu' => !empty($get_group_menu) ? $get_group_menu->id : 0,
                    'industry_token_login'  => $token_id,
                    'industry_id' => $get_data_user->id,
                    'industry_image' => $get_data_user->image,
                    'industry_name' => $get_data_user->name,
                    'industry_email' => $get_data_user->email,
                    'industry_last_check_admin' => time()
                ];
